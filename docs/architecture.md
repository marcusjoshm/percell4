...
**Tests are split by directory, not by marker.** `tests/` holds 296 test files and 4,077 test functions and runs headless under `QT_QPA_PLATFORM=offscreen`; `tests_gui/` holds 16 files and 98 test functions that build real napari viewers and need a real GL context.
