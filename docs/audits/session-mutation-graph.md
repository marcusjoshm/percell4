...
| `src/percell4/interfaces/gui/main_window.py:1083` | `_on_phasor_mask_applied` (Apply Visible as Mask handler) | `phasor_plot.apply_visible_as_mask_button` | Creator | Compliant | Writes one mask per ROI; auto-selects the last one. Per OQ-2 per-slot emission. |
