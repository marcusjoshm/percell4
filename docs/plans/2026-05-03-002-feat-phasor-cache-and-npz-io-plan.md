...
- **`.npz` schema is fixed at v1.** Top-level keys: `g`, `s` (required, float32); `g_filtered`, `s_filtered`, `lifetime_filtered` (optional but always written together when wavelet has been applied); `metadata` (a 0-d numpy object array containing a dict). `metadata["schema_version"] = 1`. Intensity is intentionally NOT in the schema (decay-derived; lives in `/decay/<ch>`). Round-trippable across PerCell4 instances when both source and target hold `/decay/<ch>` for the channel.
- **`metadata` as object-array dict** — `np.savez(file_obj, ..., metadata=np.array(metadata_dict, dtype=object))`. Loaded with `np.load(path, allow_pickle=True)`. `allow_pickle=True` documented as required in the import tab help text (consistent with how PerCell4 already loads ImageJ ROIs and `_seg.npy`).
