...
class DataPlotWindow(QMainWindow):
    ...
    - Subscribes to Session events for updates and selection
    """
    ...
    def __init__(self, session: Session) -> None:
        ...
        # Bootstrap from existing session state (late-joining window)
        if not self._session.df.empty:
    ...
    def _connect_signals(self) -> None:
        self._unsubs = [
            self._session.subscribe(Event.MEASUREMENTS_UPDATED, self._on_data_changed),
    ...
    def _on_data_changed(self) -> None:
        """Handle MEASUREMENTS_UPDATED / DATASET_CHANGED."""
        self._rebuild_dropdowns()
        self._refresh_plot()

    def _on_filter_changed(self) -> None:
        """Handle FILTER_CHANGED — re-render plot with filtered data."""
        self._refresh_plot()
    ...
    def _update_selection_highlights(self) -> None:
        ...
        label_ids = self._session.selected_ids
    ...
    def _on_point_clicked(self, scatter_item, points, ev) -> None:
        ...
        if ev.modifiers() & Qt.ControlModifier:
            ...
            current = set(self._session.selected_ids)
            if label_id in current:
                current.discard(label_id)
            else:
                current.add(label_id)
            self._session.set_selection(frozenset(current))
        else:
            self._session.set_selection(frozenset([label_id]))
    ...
    def _on_rect_selected(self, data_rect: QRectF) -> None:
        ...
        if selected_labels:
            self._session.set_selection(frozenset(int(lid) for lid in selected_labels))
    ...
    def eventFilter(self, obj, event) -> bool:
        ...
        if event.type() == QEvent.KeyPress and event.key() == Qt.Key_Escape:
            self._session.set_selection(frozenset())
            return True
